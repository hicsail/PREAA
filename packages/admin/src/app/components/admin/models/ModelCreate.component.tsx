  ...
  return (
    <Create>
      <SimpleForm>
        ...
        <TextInput source='litellm_params.api_base' validate={[required()]} label='Base URL of LangFlow (no training slash)' />
        <TextInput source='litellm_params.api_key' validate={[required()]} label='API Key for Langflow' />
        <SelectInput source='litellm_params.custom_llm_provider' validate={[required()]} choices={providers} label='LiteLLM Provider' />
      </SimpleForm>
    </Create>
  );
};
